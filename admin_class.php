...
Class Action {
	...
	function save_stall(){
		...
		// Check if the stall number already exists
		$chk = $this->db->query("SELECT * FROM stalls WHERE stall_no = '$stall_no'");
		
		// Error handling: Check if the query executed successfully
		if (!$chk) {
			return "Error: " . $this->db->error;  // Return any SQL error
		}

		if($chk->num_rows > 0 && empty($id)){
			return 2;  // Stall number already exists
			exit;
		}
		
		// Insert new stall or update existing one
		if(empty($id)){
			$save = $this->db->query("INSERT INTO stalls SET $data");
		} else {
			$save = $this->db->query("UPDATE stalls SET $data WHERE id = $id");
		}
		
		// Error handling for insert/update query
		if(!$save){
			return "Error: " . $this->db->error;  // Return any SQL error
		}
	...
	function delete_stall() {
		extract($_POST);
		$delete = $this->db->query("DELETE FROM stalls WHERE id = $id");
		if ($delete) {
			return 1;
		}
	}

	function save_renter(){
		extract($_POST);
	...
	function get_tdetails(){
		extract($_POST);
		$data =array();
		$tenants =$this->db->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.stall_no,h.price FROM tenants t inner join stalls h on h.id = t.stall_id where t.id = {$id} ");
